    ...
    if os.path.exists(path):
        ...
        with st.spinner("Connecting to The Post Database"):
            ...
            try:
                ...
                st.success("Resources will be Uploaded Soon")
...
def parse_posts(df, select):
    ...
    if not df.empty:
        ...
        for key in grouped_dict.keys():
            ...
            st.subheader(format_date.strftime("%d-%B-%Y"))

            df = grouped_dict[key]
            for _, row in df.iterrows():
                expander = st.expander(row["title"])
                expander.markdown(
                    f"""{row['body']} 
                                \n [Read More]({row['link']})
        ...
        st.success("Coming Soon! Explore Our Subjects")
